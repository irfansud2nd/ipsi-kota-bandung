...
const PaymentInvoice = ({ payment }: { payment: Payment }) => {
  ...
  const today = formatDate(payment.created_at, {
    withoutHour: true,
    shortYear: true,
    monthNumber: true,
  });
  ...
  if (!invoiceId) return <Loading full />;
  ...
  const splitData = () => {
    ...
    const contingentName = payment.contingent_name;
    const total = getTotalMatchCost(paidMatchBaseds);
    const date = today.replaceAll(" ", "/");

    let dataLength = builtData.length;
    const onePageLimit = 8;
    const firstPageLimit = 24;
    const lastPageLimit = 28;
    let splits: number[] = [onePageLimit];

    if (dataLength > onePageLimit) {
      splits = [firstPageLimit, lastPageLimit];
    }

    let lower = 0;

    splits.map((limit, index) => {
      const upper = lower + splits[index] - 1;
      for (let i = lower; i < upper; i += limit) {
        result.push({
          contingentName,
          total,
          invoiceId,
  ...
  const generatePdf = async () => {
    ...
    let pageLength = Math.floor(imgHeight / pageHeight);
    let position = 0;
    ...
    <div className="flex justify-center items-center">
      ...
      <div className="absolute -z-50 top-0 right-full">
        <div ref={printRef} className={`w-[1050px] h-fit`}>
          {datas.map((data, page) => (
            <div
              className="w-full h-[1485px] mx-auto font-semibold text-xl bg-gray-400"
              key={page}
            >
              <div
                ...
                <div className="flex-1">
                  ...
                  {!data.data.length ? null : (
                    <>
                      ...
                      {data.data.map((item, i) => (
                        <div
                          className="flex items-center text-lg font-medium border-b border-black"
                          key={item.id}
                        >
                  ...
                  {page == datas.length - 1 && (
                    <>
                      ...
                      {/* DESCRIPTION 2 */}
                      <div className="grid grid-cols-[auto_1fr] mt-2 text-justify text-lg">
                        <div className="w-[450px]">
                          ...
                          <p className="mt-3">
                            Setelah melakukan pembayaran harap melakukan
                            konfirmasi di web pendaftaran dengan
                            mengupload/melampirkan bukti transfer
                          </p>
                        </div>
